...
pair< size_t, size_t > IntermediateBufferAdapter::push(boost::asio::mutable_buffer out, boost::asio::const_buffer in)
{
    ...
    // we return the amount of data we output, and how much we consumed
    ...
    unsigned char const *const in_begin(static_cast< unsigned char const* >(in.data()));
    unsigned char const *in_curr(in_begin);
    ...
    if (remaining_to_output > size_t(distance(out_curr, out_end))) remaining_to_output = distance(out_curr, out_end);
    out_curr = copy(in_curr, in_curr + remaining_to_output, out_curr);
    in_curr += remaining_to_output;

    size_t const remaining_to_copy(min(distance(in_curr, in_end), distance(curr_, end_)));
    curr_ = copy(in_curr, in_curr + remaining_to_copy, curr_);
    in_curr += remaining_to_copy;

    return make_pair< size_t, size_t >(distance(out_begin, out_curr), distance(in_begin, in_curr));
}
}}
