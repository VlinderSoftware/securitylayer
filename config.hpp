...
namespace DNP3SAv6 {
	struct Config
	{
        ...
        std::uint8_t encryption_algorithm_ = 0/*NULL*/;
		...
		static unsigned int const max_key_wrap_data_size__ = 152; // some reasonable size for key-wrap data (it's currently 144 bytes for the data plus 8 bytes for the IV). Note that a buffer for this is allocated on the stack, so we need this to be small

		static unsigned int const max_apdu_size__ = 4096;
		static unsigned int const max_spdu_size__ = 4096;
        static unsigned int const work_buffer_size__ = 4096;
